...
const SUPPORTED_EXTENSIONS = [
  ".pdf", ".jpg", ".jpeg", ".png", ".doc", ".docx", ".xls", ".xlsx", ".csv", ".tiff", ".gif", ".xml"
];
...
export async function createXeroAttachment(
  ...
  try {
    ...
    if (!SUPPORTED_EXTENSIONS.includes(ext.toLowerCase())) {
      throw new Error(`File type ${ext} is not supported by Xero. Supported: ${SUPPORTED_EXTENSIONS.join(", ")}`);
    }
    ...
    console.log(`Upload parameters:`, { finalFileName, mimeType, entityType, entityId });
    ...
    switch (entityType.toLowerCase()) {
      case "invoices":
        sdkResponse = await xeroClient.accountingApi.createInvoiceAttachmentByFileName(
          xeroClient.tenantId,
          entityId,
          finalFileName,
        ...
        break;
      case "contacts":
        sdkResponse = await xeroClient.accountingApi.createContactAttachmentByFileName(
          xeroClient.tenantId,
          entityId,
          finalFileName,
          Buffer.from(response.data)
        ...
        break;
      case "creditnotes":
        sdkResponse = await xeroClient.accountingApi.createCreditNoteAttachmentByFileName(
          xeroClient.tenantId,
          entityId,
          finalFileName,
          Buffer.from(response.data)
      ...
      case "banktransactions":
        sdkResponse = await xeroClient.accountingApi.createBankTransactionAttachmentByFileName(
          xeroClient.tenantId,
          entityId,
          finalFileName,
          Buffer.from(response.data)
        ...
        break;
      case "manualjournals":
        sdkResponse = await xeroClient.accountingApi.createManualJournalAttachmentByFileName(
          xeroClient.tenantId,
          entityId,
          finalFileName,
          Buffer.from(response.data)
        ...
        break;
      case "receipts":
        sdkResponse = await xeroClient.accountingApi.createReceiptAttachmentByFileName(
          xeroClient.tenantId,
          entityId,
          finalFileName,
          Buffer.from(response.data)
        ...
        break;
      case "accounts":
        sdkResponse = await xeroClient.accountingApi.createAccountAttachmentByFileName(
          xeroClient.tenantId,
          entityId,
          finalFileName,
          Buffer.from(response.data)
        ...
        break;
      default:
        throw new Error(`Attachment upload not supported for entity type: ${entityType}`);
    }
