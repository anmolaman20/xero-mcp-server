...
const CreateAttachmentTool = CreateXeroTool(
  ...
  "Upload an attachment to a Xero entity (invoice, contact, credit note, etc.). Only supported file types: PDF, JPG, JPEG, PNG, DOC, DOCX, XLS, XLSX, CSV, TIFF, GIF, XML.",
  {
    ...
    entityId: z.string().describe("The ID of the entity to upload the attachment to."),
    fileUrl: z.string().url().describe("The public URL of the file to upload."),
    fileName: z.string().optional().describe("The file name to use for the attachment (optional, will use from URL if not provided)."),
  },
  async ({ entityType, entityId, fileUrl, fileName }) => {
    const result = await createXeroAttachment(entityType, entityId, fileUrl, fileName);
    if (result.isError) {
      return {
        content: [
          {
